...
export default function GalleryAdmin() {
  ...
  async function create(fd: FormData) {
    ...
    setMsg(r.ok ? 'Альбом создан' : 'Ошибка при создании альбома');
    if (r.ok) load();
  }
  ...
  async function uploadFiles(files: FileList | File[]) {
    ...
    const r = await fetch('/api/admin/photos/upload', { method: 'POST', body: fd });
    if (!r.ok) return setMsg('Ошибка загрузки файлов');

    const data = await r.json();
    setMsg(`Загружено ${data.createdCount} из ${data.requestedCount} файлов`);
  }
  ...
  return (
    <div className="card">
      ...
      <form action={create}>
        <input name="title" placeholder="Новый альбом" required /> <button>Добавить</button>
      </form>
      ...
      <div
        ...
        onDrop={onDrop}
        style={{ marginTop: 12, padding: 16, border: '2px dashed #c7b49a', borderRadius: 10 }}
      >
